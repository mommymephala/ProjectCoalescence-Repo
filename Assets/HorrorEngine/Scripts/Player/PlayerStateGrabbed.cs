...
namespace HorrorEngine
{
    public class PlayerStateGrabbed : ActorStateWithDuration
    {
        ...
        private void UpdatePositioning()
        {
            ...
            {
                var targetPos = m_Grabber.transform.TransformPoint(positioning.Offset);
                if (NavMesh.SamplePosition(targetPos, out NavMeshHit hit, 5f, NavMesh.AllAreas))
                {
                    m_GroundDetect.Detect(hit.position);
                    targetPos = m_GroundDetect.Position;
                    Actor.transform.position = Vector3.MoveTowards(Actor.transform.position, targetPos, positioning.MoveSpeed * Time.fixedDeltaTime);
                }
            }
        }
